...
import { deleteFromGroceries } from "../../services/userService";

// https://codepen.io/davidmunro/pen/xxxeoOo
const AccordionBoodschappen = ({ title, children, me, setMe, id, dayall }) => {
  const [isOpen, setOpen] = useState(false);
  // console.log("dayall");
  // console.log(dayall);
  return (
    <div className="accordion-wrapper mt-36">
      <div className={`accordion-title bg-white ${isOpen ? "open" : ""}`}>
        <div className="">
          {title}
          <span
            className="text-verwijder"
            onClick={() => deleteFromGroceries(me, setMe, id)}
          >
            &nbsp;
...
export default AccordionBoodschappen;
